...
export default function ProfilePostItem({ post, setIsUpdate }) {
  ...
  useEffect(() => {
    if (isLikesModalOpen) {
      axios.get(`https://dev-azproduction-api.flynautstaging.com/settings/postLikes/${post.id}`, {
        headers: {
          Authorization: sessionStorage.getItem("accessToken")
        }
      }).then((res) => {
  ...
  useEffect(() => {
    if (isCommentsModalOpen) {
      axios.get(`https://dev-azproduction-api.flynautstaging.com/settings/postComments/${post.id}`, {
        headers: {
          Authorization: sessionStorage.getItem("accessToken")
        }
      }).then((res) => {
  ...
  //     }}
  //   >
  //     <Avatar src={settings?.photoURL} alt={settings?.displayName} />
