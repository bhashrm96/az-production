...
export default function FeedsQuickEditForm({ currentFeeds, open, onClose }) {
  ...
  const NewFeedsSchema = Yup.object().shape({
  ...
  const defaultValues = useMemo(
    () => ({
      firstname: currentFeeds?.firstname || '',
      email: currentFeeds?.email || '',
      phone: currentFeeds?.phone || '',
      lastname: currentFeeds?.lastname || '',
      company_address: currentFeeds?.company_address || '',
      dob: currentFeeds?.dob || '',
      city: currentFeeds?.city || '',
      state: currentFeeds?.state || '',
      zipcode: currentFeeds?.zipcode || '',
      company_name: currentFeeds?.company_name || '',
      website: currentFeeds?.website || '',
      company_about: currentFeeds?.company_about || '',
      position: currentFeeds?.position || '',
    }),
    [currentFeeds]
  );

  const methods = useForm({
    resolver: yupResolver(NewFeedsSchema),
    defaultValues,
  });
            ...
            {permissions.map((permission, index) => (
              <>
                ...
                >
                  ...
                  <MenuItem value="6">Feeds</MenuItem>
                </RHFSelect>
                <RHFSelect
                  name="permission"
...
FeedsQuickEditForm.propTypes = {
  currentFeeds: PropTypes.object,
