...
export default function UserNewEditForm({ currentUser }) {
  ...
  const NewUserSchema = Yup.object().shape({
    hashtag: Yup.string().required('Hashtag is required').matches(/^#[^\s#]*$/, 'Hashtag not if proper format (for e.g. #films)'),
  });
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      await axios.post('https://dev-azproduction-api.flynautstaging.com/admin/createHashtag', body).then(() => {
        router.push(paths.dashboard.categoryManagement.feeds);
      }).catch((err) => {
        console.log(err);
      })
    } catch (error) {
          ...
          <Card sx={{ p: 3 }}>
            ...
            <Stack alignItems="flex-end" sx={{ mt: 3 }}>
              <LoadingButton type="submit" variant="contained" loading={isSubmitting}>
                {!currentUser ? 'Create Hashtag' : 'Save Changes'}
              </LoadingButton>
            </Stack>
          </Card>
        </Grid>
