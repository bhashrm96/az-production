...
import UserQuickEditForm from './management-hashtags-quick-edit-form';

// ----------------------------------------------------------------------

export default function UserTableRow({ row, selected, onEditRow, onSelectRow, setIsUpdate }) {
  const { hashtagText } = row;
  ...
  const handleStatusChange = async (id) => {
    const res = await axios.delete(`https://dev-azproduction-api.flynautstaging.com/admin/deleteHashtags/${id}`, {
      headers: {
  ...
  return (
    <>
      ...
      <ConfirmDialog
        ...
        action={
          <Button variant="contained" color="error" onClick={() => handleStatusChange(row.hashtagId)}>
            Delete
          </Button >
        }
      />
