...
export default function CommunityNewEditForm({ currentCommunity }) {
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      const response = await axios.post('https://dev-azproduction-api.flynautstaging.com/community/create_community', body);
      if (response.status === 201) {
        router.push(paths.dashboard.community.list);
      }
    } catch (error) {
            ...
            <RHFSwitch
              ...
              label={
                <>
                  ...
                  <Typography variant="body2" sx={{ color: 'text.secondary' }}>
                    Disabling this will automatically send the community a verification email
                  </Typography>
                </>
              }
          ...
          <Card sx={{ p: 3 }}>
            ...
            <Stack alignItems="flex-end" sx={{ mt: 3 }}>
              <LoadingButton type="submit" variant="contained" loading={isSubmitting}>
                {!currentCommunity ? 'Create Community' : 'Save Changes'}
              </LoadingButton>
            </Stack>
          </Card>
        </Grid>
...
CommunityNewEditForm.propTypes = {
