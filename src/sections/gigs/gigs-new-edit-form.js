...
export default function GigsNewEditForm({ currentGigs }) {
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      const response = await axios.post('https://dev-azproduction-api.flynautstaging.com/gigs/create_gigs', body);
      if (response.status === 201) {
        router.push(paths.dashboard.gigs.list);
      }
    } catch (error) {
            ...
            <RHFSwitch
              ...
              label={
                <>
                  ...
                  <Typography variant="body2" sx={{ color: 'text.secondary' }}>
                    Disabling this will automatically send the gigs a verification email
                  </Typography>
                </>
              }
          ...
          <Card sx={{ p: 3 }}>
            ...
            <Stack alignItems="flex-end" sx={{ mt: 3 }}>
              <LoadingButton type="submit" variant="contained" loading={isSubmitting}>
                {!currentGigs ? 'Create Gigs' : 'Save Changes'}
              </LoadingButton>
            </Stack>
          </Card>
        </Grid>
