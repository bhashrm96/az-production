...
import { _roles, _eventsList, EVENTS_STATUS_OPTIONS } from 'src/_mock';
...
const STATUS_OPTIONS = [{ value: 'all', label: 'All' }, ...EVENTS_STATUS_OPTIONS];
...
export default function EventsListView() {
  ...
  useEffect(() => {
    axios.get("https://dev-azproduction-api.flynautstaging.com/admin/all-events").then((res) => {
            ...
            {STATUS_OPTIONS.map((tab) => (
              <Tab
                ...
                icon={
                  ...
                  >
                    {tab.value === 'all' && _eventsList.length}
                    {tab.value === 'active' &&
                      _eventsList.filter((events) => events.status === 'active').length}

                    {tab.value === 'pending' &&
                      _eventsList.filter((events) => events.status === 'pending').length}
                    {tab.value === 'banned' &&
                      _eventsList.filter((events) => events.status === 'banned').length}
                    {tab.value === 'rejected' &&
                      _eventsList.filter((events) => events.status === 'rejected').length}
                  </Label>
                }
              />
            ))}
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (name) {
    inputData = inputData.filter(
      (events) => events.name.toLowerCase().indexOf(name.toLowerCase()) !== -1
    );
  }

  if (status !== 'all') {
    inputData = inputData.filter((events) => events.status === status);
  }

  if (role.length) {
    inputData = inputData.filter((events) => role.includes(events.role));
  }
