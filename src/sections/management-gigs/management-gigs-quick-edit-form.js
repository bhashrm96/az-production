...
export default function UserQuickEditForm({ currentUser, open, onClose, setIsUpdate }) {
  ...
  const [old, setOld] = useState(currentUser.gigs_category_name)
  const [permissions, setPermissions] = useState([]);
  const [gender, setGender] = useState([]);

  useEffect(() => {
    setOld(currentUser.gigs_category_name)
  }, [currentUser])
  ...
  const defaultValues = useMemo(
    () => ({
      category: currentUser?.gigs_category_name || ''
    }),
    [currentUser]
  );
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      await axios.post(`https://dev-azproduction-api.flynautstaging.com/admin/createGigsCategory/`, body).then(() => {
        onClose();
