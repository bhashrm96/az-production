...
export default function ClassifiedNewEditForm({ currentClassified }) {
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      const response = await axios.post('https://dev-azproduction-api.flynautstaging.com/admin/create-classified', body);
      if (response.status === 201) {
        router.push(paths.dashboard.classified.list);
      }
    } catch (error) {
            ...
            <RHFSwitch
              ...
              label={
                <>
                  ...
                  <Typography variant="body2" sx={{ color: 'text.secondary' }}>
                    Disabling this will automatically send the classified a verification email
                  </Typography>
                </>
              }
          ...
          <Card sx={{ p: 3 }}>
            ...
            <Stack alignItems="flex-end" sx={{ mt: 3 }}>
              <LoadingButton type="submit" variant="contained" loading={isSubmitting}>
                {!currentClassified ? 'Create Classified' : 'Save Changes'}
              </LoadingButton>
            </Stack>
          </Card>
        </Grid>
...
ClassifiedNewEditForm.propTypes = {
