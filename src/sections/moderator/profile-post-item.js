...
import { useMockedModerator } from 'src/hooks/use-mocked-moderator';
...
export default function ProfilePostItem({ post }) {
  const { moderator } = useMockedModerator();
  ...
  const renderHead = (
    <CardHeader
      disableTypography
      avatar={<Avatar src={moderator?.photoURL} alt={moderator?.displayName} />}
      title={
  ...
  const renderInput = (
    ...
    >
      <Avatar src={moderator?.photoURL} alt={moderator?.displayName} />
